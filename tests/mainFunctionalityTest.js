...
async function mainFunctionalityTest(page) {
    ...
    const expectedLocalStorageContent = {
        "reservations": "[{\"device_id\":3,\"device_type\":\"Device C\",\"device_version\":\"2.2.0\",\"reservation_time\":\"2024-12-26T19:59:44.962Z\",\"reservation_duration\":24,\"reservation_reason\":\"Good reason! (Edited)\"}]",
    ...
    const actualReservations = JSON.parse(actualLocalStorageContent["reservations"]);
    const expectedReservations = JSON.parse(expectedLocalStorageContent["reservations"]);
    ...
    const compareReservations = (actual, expected) => {
        return actual.every((reservation, index) => {
            return (
                ...
                reservation.reservation_duration === expected[index].reservation_duration &&
                reservation.reservation_reason === expected[index].reservation_reason &&
                isValidDateString(reservation.reservation_time)
            );
        });
    };
    await expect(compareReservations(actualReservations, expectedReservations)).toBe(true);
    await expect(actualLocalStorageContent["first-time-login"]).toBe(expectedLocalStorageContent["first-time-login"]);
}
